...
func SearchTasks(db *sql.DB, searchKey string) (task.TaskList, error) {
	if db == nil { return nil, fmt.Errorf("searching tasks: cannot search a nil database") }
	if searchKey == "" { return nil, fmt.Errorf("searching tasks: cannot search without a search term") }
	...
	if err != nil { return nil, fmt.Errorf("searching tasks: %s", err) } 
	defer rows.Close()
	...
	if err = rows.Err(); err != nil { return nil, fmt.Errorf("searching tasks: %s", err) }

	return matches, nil
}

func SearchTasksMetaData(db *sql.DB, searchKey string) (task.TaskMetaDataList, error) {
	if db == nil { return nil, fmt.Errorf("searching task meta data: cannot search a nil database") }
	if searchKey == "" { return nil, fmt.Errorf("searching task meta data: cannot search without a search term") }

	rows, err := db.Query(`SELECT title, COUNT(*), MAX(completion_date) FROM task WHERE title LIKE ? GROUP BY title`, "%"+searchKey+"%")
	if err != nil { return nil, fmt.Errorf("searching task meta data: %s", err) }
	...
	if err = rows.Err(); err != nil { return nil, fmt.Errorf("searching task meta data: %s", err) }
